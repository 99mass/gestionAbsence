<?php 
                /**
                 * dans cette page on a le requete pour afficher la liste de presence
                 */

         @session_start();    
            /**
             * liste_present() permet de afficher la liste d'absence en fonction du mois  et du jour ou du jour seule
             *
             * @param  $jour
             * @param  $mois
             * @param  $niveau
             ...
             * @return void
             */
            function liste_present($jour,$mois,$niveau,$serie,$classe,$data_base){
             $sql = "SELECT  {$niveau}_{$serie}_{$classe}.id, prenom,nom, DATE_FORMAT(date_naissance, '%d/%m/%Y') AS date_naissances,
                    ...
                    WHERE  presence='present' AND  DATE_FORMAT(date_cours, '%d')={$jour} AND DATE_FORMAT(date_cours, '%m')={$mois}
             ...
             function liste_present_sans_le_jour($mois,$niveau,$serie,$classe,$data_base){
             $sql = "SELECT  {$niveau}_{$serie}_{$classe}.id, prenom,nom, DATE_FORMAT(date_naissance, '%d/%m/%Y') AS date_naissances,
                            mp_{$niveau}_{$serie}_{$classe}.id AS id_FK,presence,DATE_FORMAT(date_cours, '%d/%m/%Y') AS date_cour,
                            TIME_FORMAT(heure_cours,'%Hh%imn') AS heure,cours
                    FROM  {$niveau}_{$serie}_{$classe} 
                    INNER JOIN mp_{$niveau}_{$serie}_{$classe} ON {$niveau}_{$serie}_{$classe}.id= mp_{$niveau}_{$serie}_{$classe}.id_proprietaire
                    WHERE  presence='present' AND DATE_FORMAT(date_cours, '%m')={$mois} 
               ...
               $jour_chercher=htmlspecialchars(trim(strip_tags($_POST['jour_chercher'])));
                        if ($jour_chercher==0 && $moisChercher!=0) {
                            // $jour_chercher=$jourActuel;
                             $liste_present=liste_present_sans_le_jour($moisChercher,$niveaux,$series,$classes,$db);
                        }
                         else   if ($jour_chercher!=0 && $moisChercher==0) {
                            $moisChercher=$moisActuel;
                            $liste_present=liste_present($jour_chercher,$moisChercher,$niveaux,$series,$classes,$db);
                        }else {
                             $liste_present=liste_present($jour_chercher,$moisChercher,$niveaux,$series,$classes,$db);
                        }
                    ...
                    $liste_present=liste_present($jourActuel,$moisActuel,$niveaux,$series,$classes,$db);
                  }else {
                        $liste_present=liste_present_sans_le_jour($moisActuel,$niveaux,$series,$classes,$db);                    
                  }
